...
private :
	...
	u64 interchar_timer_duration = 50000; //500ms

	void start_interchar_timer()		{ interchar_start_time = system_time(); }
	bool is_interchar_timer_expired() 	{ if(system_time() - interchar_start_time > interchar_timer_duration){ return true; } return false; }
	...
	*/
	...
	void print_i(float f) {
		u8 size = 4;	//number of bytes the float occupies	
		u8 the_float[size];
