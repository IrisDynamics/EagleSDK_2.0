...
public:
	...
	u32 up_count = 0;
	u32 down_count = 0;
	volatile states state = listening;
...
//		UART_Init(uart_ptr, &uartConfig, 120000000U);
	...
	void tdre_isr() {
		switch(state) {
			...
			case listening:
				orca_mem.write(LISTEN_TDRE_ERROR, orca_mem.read(LISTEN_TDRE_ERROR) + 1);
				break;
				...
				for (u16 i = 0; i < tx_space; i++) {
					if(get_tx_buffer_size()) {				// if there is data in the tx buffer, send it
						...
						up_count++;
							...
							|| is_disconnected()
							) { // If there is no data in the tx buffer and the GUI frame is finished building or the device is disconnected, enable TC and disable TDRE
	...
	void tc_isr() {
		switch(state) {
			...
			case listening:
				orca_mem.write(LISTEN_TC_ERROR, orca_mem.read(LISTEN_TC_ERROR) + 1);
				break;
			...
			case tx:
				...
				// decide next state based on connection status
				if(is_connected()) {
					state = rx;	// back to rx if still connected
				}
				else {
					state = listening; // back to listening if not connected
				}
