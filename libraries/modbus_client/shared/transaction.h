...
class Transaction {
    ...
    enum TRANSMIT_STATE {
    	...
    	queued,					// has been loaded with data to send, but hasnt been marked as transmitted
		sent,					// has been transmitted (or is transmitting), but not marked as received or timed out
		finished,				// marked as done (either received or error ecnountered)rked as received or timed out
...
public:
    ...
    */
    ...
    /**
     * @breif true when is a broadcast (ie address is 0)
     */
     ...
     * @param id the intiger id of the request
    */
    void set_ID(){
        ID = id_assigner++;
    }
     ...
     * @return and intiger id that represents the request
    */
    int get_ID(){
        return ID;
    }
