 ...
 * # Intercharacter timeout - starts after receiving a byte in the receiving state, is cleared when receiving the message's known payload, and expiry invalidates messages of known size, and triggers validation of unknown-seze messages
 * # Interframe delay - starts following validation/invalidation of a message, is cleared only when it expires, and expirey returns the client to Idle
 * # Broadcast delay - starts following transmission of a broadcast message, is cleared when expireing and expirey enables the interframe delay
 *
 * */
class ModbusClient {
    ...
    void run_in() {
    	...
    	// A timer is enabled
			...
			case TIMER_ID::interchar_timeout:
				...
				// If the length was known and an interchar timeout occurred (ie the message got fucked)
				else {
...
///////////////////////////////////////////////////////////
	...
	void receive() {
		...
		active_transaction->load_reception(byte); //read the next byte from the receiver buffer. This clears the byte received interrupt    ??should we be loading here? it seems that in the overrun case we've already walked off the end of the array??
