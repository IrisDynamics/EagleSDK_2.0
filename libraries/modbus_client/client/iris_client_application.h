...
public:
    ...
    */
    int set_connection_config(IrisClientApplication::ConnectionConfig config){
        ...
        if(config.server_address < 1 || config.server_address > 247){
            return 0;   //we need to go through and standardize what our error value is
        }

        connection_config = config;

        return 1;   //we need to go through and standardize what our error value is
    }
    ...
    */
    void disable(){
        enabled = false;
        if(is_connected()){
            enqueue_change_connection_status_fn(connection_config.server_address, false, 0, 0); //not so sure about this change!
	...
	void modbus_handshake() {
		...
		case synchronization:
			...
			if (new_data() && !response->is_reception_valid()) {
				// this should allow all the crap we put on the queue to timeout or be received before reattempting a connection
				disconnect();
			}
			else {
