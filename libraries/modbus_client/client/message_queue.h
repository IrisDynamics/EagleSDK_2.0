...
public:
    ...
    */
    ...
    /**
     * @brief returns a pointer to the active transaction. No checkign is done as to the state of the transaction
     */
     ...
     * When this returns true, the transaction at active_index is ready to transmit, but hasnt been started yet
     */
	...
	if ((active_index == back_index) || transaction_buffer[active_index].is_active()) {
		...
		// current message hasnt been sent yet, but was loaded and queued up
		ret = true;
