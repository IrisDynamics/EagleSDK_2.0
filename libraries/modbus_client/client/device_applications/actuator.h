/**
   ...
   @brief  Actuator object that abstracts the use of the modbus library/ communication with an Orca motor server
   Additional documentation about the use of this object can be found in the Orca API reference manual
   https://www.irisdynamics.com/wp-content/uploads/Orca-API-Reference-Manual.pdf
   @version 1.0
...
public: 
	...
	*/
	...
	/**
	*@brief Get to a good handshake init state and set up teh device driver with the default baud rate
	*/
	void init(){
	 ...
	 * This function must be exteranlly paced...ie called at the frequency that transmission should be sent
	 */
	void run_out() {
	...
	*/
	void set_pctrl_tune_softstart(uint16_t t_in_ms){
		write_single_register_fn(1,SOFTSTART_PERIOD 	,t_in_ms);
	}
	...
	void tune_position_controller(uint16_t pgain, uint16_t igain, uint16_t dvgain, uint32_t sat, uint16_t degain=0) {
		...
		write_single_register_fn	(1, PC_GAIN_SCHEDULE_DISABLE, 1);
		write_single_register_fn	(1, CONTROL_REG_1::address, CONTROL_REG_1::position_controller_gain_set_flag);

	}

//	/**
//	 * @brief Sets the PID tuning values on the motor using the gain scheduling.Enables gain scheduling
//	 *
//	 * @param pgain proportional gain
//	 * @param igain integral gain
//	 * @param dgain derivative gain
//	 * @param sat maximum force (safety value)
//	 */
//	void tune_position_controller(uint16_t pgain, uint16_t igain, uint16_t dgain, uint16_t pgain_ls, uint16_t igain_ls, uint16_t dgain_ls, uint32_t sat) {
//
//		uint8_t data[12] = {
//				uint8_t(pgain>>8),
//				uint8_t(pgain),
...
//
//		write_multiple_registers_fn	(1, PC_PGAIN, 6,  data);
//		write_single_register_fn	(1, PC_GAIN_SCHEDULE_DISABLE, 0);
//		write_single_register_fn	(1, CONTROL_REG_1::address, CONTROL_REG_1::position_controller_gain_set_flag);
//
//	}
