/**
   ...
   @author Rebecca McWilliam Kali Erickson <kerickson@irisdynamics.com>
...
//
   ...
   {}
	...
	/// returns the last-known memory contents pf the passed register
	...
	/*
	 * @brief Seek to reach a target pressure, hysterisis to not bounce around
	...
	*/
	...
	void run_in() {
		...
		if ( modbus_client.is_response_ready() ) {
			...
			new_data_flag = true;		// communicate to other layers that new data was receivede
...
//		//read system parameter registers !
	...
	/*
	 * @brief enqueue a frame that will read sensor values cycle between the relevant registers, so dont need to do giant read at once
	 */
	...
	/**
	 * @brief return teh number of failed messages
	 */
