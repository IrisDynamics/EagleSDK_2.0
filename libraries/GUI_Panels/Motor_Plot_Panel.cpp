...
*/
void Motor_Plot::add(Actuator * _motor, const  char * _name, uint16_t _anchor_row, uint16_t _anchor_col, uint16_t _plot_height=12, uint16_t _plot_width=22){
...
*/ 
void Motor_Plot::add(GUI_Page * _plot_elements, Actuator * _motor, const  char * _name, uint16_t _anchor_row, uint16_t _anchor_col, uint16_t _plot_height=12, uint16_t _plot_width=22){
...
void Motor_Plot::run() {
	...
	switch (log_state) {
	case start:
		filename = (String(motor->get_serial_number()) + "_memory_log").c_str();
	...
	case writing:
		for (int i = 0; i < ORCA_REG_SIZE; i++) {
			write_string = (String(i) + "\t" + String(motor->get_orca_reg_content(i))).c_str();
