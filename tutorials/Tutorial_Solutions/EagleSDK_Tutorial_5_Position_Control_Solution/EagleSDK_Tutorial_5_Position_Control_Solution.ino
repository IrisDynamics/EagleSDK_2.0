 ...
 */
void setup() {  
    motor.init();                                       // Initialize the UART port on the Eagle to allow communications with a motor (enabling interrupts and setting a default baud rate)
    ...
    */
    ...
    connection_config.target_delay_us = 0;              // Units =  microseconds 
...
void loop() {
    ...
    // Set force using console command
    motor.set_force_mN(gui.target_force);
    
    // Set position using signal panel
